...
c     subroutine  NLSS                                      
c
c     Non-Linear Least Square optimization, smooth solution
c
c-----------------------------------------------------------------------
      SUBROUTINE nlss ( simext, mxopt, xini, nfct, obs, spadim, grid,
     &                  epsbfg, musmo, matsmo,
...
c            dwork  : vector( mxopt*(3*mxopt+21)/2                double
...
c     external functions
      EXTERNAL simext, simnlss, YV, IVX, PMX, YVI, bfgsboxs
c
c     arguments i/o
      INTEGER mxopt, nfct, spadim, liudat, ldudat, info
      INTEGER iudata(*)
      DOUBLE PRECISION epsbfg, musmo
      DOUBLE PRECISION obs(*), xini(*), grid(*), matsmo(*), dudata(*),
     &                 xopt(*)
c
c     workspaces
      INTEGER iwork(*)
...
c     local variables
     ...
     &        pbinf, pbsup, pgrad, pdwbf,
     &        pwsnls, pwsext, pobs, pgrid, psmo, pdwu
      DOUBLE PRECISION infini, funct
...
c          = mxopt*(3*mxopt+21)/2 + nfct*(mxopt+3+spadim) + ldudat
c
c-----------------------------------------------------------------------
c
...
c
c     optimization BFGS
      CALL bfgsboxs ( simnlss, simext, mxopt, xopt, epsbfg,
