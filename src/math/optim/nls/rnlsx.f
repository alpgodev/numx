...
c     subroutine RNLSX                                      
c
c     Robust Non-Linear Least-Square Solver - Expert version
c
c------------------------------------------------------------------------
      SUBROUTINE RNLSX ( SIMEXT, mxpar, xini, dxmin, df1, epsabs,
...
c     arguments i/o
     ...
     &        liudat, ldudat, info
      INTEGER iudata(*)
      DOUBLE PRECISION df1, epsabs, skobs
...
c
c     local variables
      INTEGER mxopt, piwbf, pnobs, pspad, pliu, pldu, pinfo, piwu,
     &        pdwbf, pwsnls, pwsext, piwini, pdwini, pbinf, pbsup,
     &        pdxmin, pgrid, psk, palpha, pbeta, pvp, prspec, prcov,
     &        pdwu, pxsol, pgrad, pxini
...
c     pbsup : pointer for superior bounds bsup, so mxopt more
      pdxmin = pbsup + ( mxopt )
c     pdxmin : pointer for superior bounds dxmin, so mxopt more      
      pgrad = pdxmin + ( mxopt )
...
c     = mxopt*(mxopt+17)/2 + nobs*(7*nobs+mxpar+spadim+33) + 2 + ldudat
c            with mxopt = mxpar + 1
c
c-----------------------------------------------------------------------
c
...
c     optimization BFGS-box
      CALL BFGSBOX ( SIMRNLS, SIMEXT, mxopt, dwork(pxini),
     &        dwork(pdxmin), df1, epsabs, mode, maxitr, maxsim,
     &        dwork(pbinf), dwork(pbsup), iwork(piwbf), dwork(pdwbf),
     &        dwork(pxsol), fxopt, dwork(pgrad), totitr, totsim, info )
c
c     optimal point
