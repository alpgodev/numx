 ...
 --------------------------------------------------------------------------*/
{
	...
	if ((j < (*p)) || (j > (*p))) {
		*info = -9000; /* error: sum(block[i]) != p*/
		return 5;
	}

	/* test if size block[i] < nb factors */
	for (i=0; i<(*nblock); i++){
		if (block[i] < *k) {
			*info = -8000+i; /* error: size block[i] < nb factors */
			return 5;
		}
	}
    ...
    */
    ...
    /* initialize F and FF matrix */
    a = 0.0;
	imx_( p, &m, F, &a );
	imx_( p, &m, FF, &a );

	/** cross-correlation G(m,m), m = nblock*k
     *
	 *       |   Id       Z(1,2) ... Z(1,nblock) |
	 *       | Z(2,1)      Id    ... Z(2,nblock) |
	 *   G = |   .         .     ...      .      |
	 ...
	 *       | Z(nblock,1) .     ...      Id     |
     *
     *	Z(i,j) the cross correlation between block i and j
     *
     *  Z(i,j) = inv(sqrt(Di))*Fi'*Corr(i,j)*Fj*inv(sqrt(Dj))
     */
	G = (double *) malloc(m*m*sizeof(double));
	a = 1.0;
	imdx_( &m, G, &a ); /* matrix identity initialization */
	...
	for (i=0; i<(*nblock)-1; i++){
		...
		for (j=i+1; j<(*nblock); j++){
			...
			/* dwork(block[i],k) = Corr(block[i],block[j])*Fj(block[j],k) */
	...
	/* compute Choleski matrix of cross-correlation matrix: chol(G) */
