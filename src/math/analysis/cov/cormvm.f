...
c
c-----------------------------------------------------------------------
      SUBROUTINE cormvm ( n, p, X, dwork, rho, corr, info)
c-----------------------------------------------------------------------
c
c     INPUT 
...
c     arguments i/o
      INTEGER n, p, info
      DOUBLE PRECISION X(n, *), rho(*), corr(p,*)
c
c     workspaces     
      DOUBLE PRECISION dwork(*)
c
c     local variables
      INTEGER pdwork, pcov, psqr, i, j, kij, kii
...
c     empirical covariance matrix
      CALL empcov ( n, p, X, rho, dwork(pdwork), dwork(pcov), info )
      IF (info .LT. 0) RETURN
c
