c=======================================================================
c     subroutine BAYESSTEIN
c
c     Bayes-Stein expected return 
c     shrinkage estimator (introduced by Jorion)
c     
...
c 
c----------------------------------------------------------------------
      SUBROUTINE bayesstein (n, p, cov, rho, iwork, dwork, mu, info)
c----------------------------------------------------------------------
c
c     INPUT
...
c
c     OUTPUT 
c       mu    : Bayes-Stein estimator (p)                       double
...
c     local variables
     ...
     &        pdv, pdones
      DOUBLE PRECISION x, y, t, w, epsbfg, alpha, EPS, ZERO, ONE
...
c     target (1'*inv(cov)*rho)/(1'*inv(cov)*1)     
      t = x/y 
c
c     shrinkage intensity (p+2)/(p+2+n*(rho-t)'*inv(cov)*(rho-t))
      DO i = 1,p
