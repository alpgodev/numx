...
C***FIRST EXECUTABLE STATEMENT  DXADD
   ...
   10 IF (Y.NE.0.0D0) GO TO 20
      ...
      GO TO 220
   20 CONTINUE
      IF (IX.GE.0 .AND. IY.GE.0) GOTO 40
      IF (IX.LT.0 .AND. IY.LT.0) GOTO 40
      IF (ABS(IX).LE.6*L .AND. ABS(IY).LE.6*L) GOTO 40
      IF (IX.GE.0) GOTO 30
      Z = Y
      IZ = IY
      GO TO 220
   ...
   40 I = IX - IY
      ...
      IF (I .EQ. 0) GOTO 50
      IF (I .GT. 0) GOTO 90
   50 IF (ABS(X).GT.1.0D0 .AND. ABS(Y).GT.1.0D0) GOTO 60
      IF (ABS(X).LT.1.0D0 .AND. ABS(Y).LT.1.0D0) GOTO 70
      Z = X + Y
      IZ = IX
      GO TO 220
   60 S = X/RADIXL
